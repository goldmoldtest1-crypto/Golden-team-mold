...
export function HomePage() {
  ...
  const Logo = ({ size = "w-10 h-10", gradientId = "logo-gradient" }) => (
    <div className={`relative ${size} flex items-center justify-center`}>
      <svg viewBox="0 0 100 100" className="w-full h-full fill-none stroke-[6px]" style={{ stroke: `url(#${gradientId})` }}>
        <defs>
    ...
    <div className="flex flex-col min-h-screen font-sans text-slate-900 bg-[#f8fffd]">
      ...
      <nav className="flex items-center justify-between px-6 py-4 border-b border-teal-100 bg-white sticky top-0 z-50 shadow-sm">
        ...
        <div className="hidden md:flex items-center gap-6 font-semibold text-sm uppercase tracking-wider text-[#2d4a53]">
          ...
          <a href="#contact" className="px-6 py-2 bg-[#2d4a53] text-white rounded-full hover:bg-[#48c9b0] transition-all text-xs">Contact</a>
        </div>
      </nav>
      ...
      <header className="px-6 pt-12 pb-20 text-center max-w-4xl mx-auto">
        <h2 className="text-2xl md:text-3xl font-bold text-[#2d4a53] mb-4">
          ...
          <span className="text-teal-600 underline decoration-teal-200 underline-offset-8">Your health is at risk!</span>
        </h2>
        
        <div className="my-12 relative rounded-3xl overflow-hidden shadow-2xl border-8 border-white max-w-4xl mx-auto">
           <img src="https://images.unsplash.com/photo-1584622650111-993a426fbf0a?auto=format&fit=crop&q=80&w=1000" alt="Inspection" className="w-full h-[500px] object-cover" />
           <div className="absolute top-8 left-8 bg-white/95 backdrop-blur px-6 py-4 rounded-2xl shadow-2xl border border-teal-50">
              <div className="flex items-center gap-4">
                 <Logo size="w-12 h-12" gradientId="badge-gradient" />
                 <div className="flex flex-col text-left">
                    <span className="font-black text-2xl text-[#2d4a53] tracking-tighter leading-none">Golden</span>
                    <span className="font-black text-2xl text-[#2d4a53] tracking-tighter leading-none">Team</span>
                 </div>
              </div>
           </div>
        </div>
        ...
        <div className="grid grid-cols-3 gap-4 max-w-2xl mx-auto">
          {["Air Quality Testing", "Microbial Testing", "Thermal Imaging"].map((label, i) => (
            <div key={i} className="flex flex-col items-center">
              <div className="w-16 h-16 bg-white rounded-full shadow-md flex items-center justify-center mb-3 border border-teal-50">
      ...
      <section id="contact" className="px-6 py-24 max-w-4xl mx-auto">
        <div className="bg-white rounded-[3rem] shadow-2xl overflow-hidden border border-teal-50 p-8 md:p-16">
          <div className="text-center mb-12">
            <h3 className="text-3xl font-black text-[#2d4a53] uppercase mb-4">Request a Free Quote</h3>
            <p className="text-slate-600">Fill out the form below and an expert will contact you shortly.</p>
          </div>
          ...
          {submitted ? (
            <div className="bg-teal-50 p-12 rounded-3xl text-center border border-teal-100">
              ...
              <h4 className="text-2xl font-bold text-[#2d4a53] mb-2">Thank You!</h4>
              <p className="text-slate-600">Your request has been received. We'll be in touch soon.</p>
            </div>
          ) : (
            <form onSubmit={handleSubmit} className="space-y-6 text-left">
              <div className="grid md:grid-cols-2 gap-6">
                <input required placeholder="Full Name" className="w-full px-6 py-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-teal-500 outline-none" value={formData.name} onChange={e => setFormData({...formData, name: e.target.value})} />
                <input required type="tel" placeholder="Phone Number" className="w-full px-6 py-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-teal-500 outline-none" value={formData.phone} onChange={e => setFormData({...formData, phone: e.target.value})} />
              </div>
              <textarea rows={4} placeholder="How can we help?" className="w-full px-6 py-4 bg-slate-50 rounded-2xl border-none focus:ring-2 focus:ring-teal-500 outline-none resize-none" value={formData.message} onChange={e => setFormData({...formData, message: e.target.value})}></textarea>
      ...
      <div className="bg-[#2d4a53] text-white py-6 px-6 text-center sticky bottom-0 z-40 shadow-[0_-10px_30px_rgba(0,0,0,0.1)]">
        <div className="max-w-4xl mx-auto flex flex-col md:flex-row items-center justify-center gap-4">
          ...
          <div className="flex items-center gap-4 text-lg md:text-xl font-bold">
             <a href="tel:8184577214" className="hover:text-teal-400 transition-colors">(818) 457-7214</a>
             <span className="text-slate-500">•</span>
             <a href="tel:4244765050" className="hover:text-teal-400 transition-colors">(424) 476-5050</a>
          </div>
        </div>
      </div>
